...
RSpec.describe Sharing, type: :model do
  ...
  it 'should allow you to update :share_by_default' do
    Sharing.share_by_defaults.keys.reverse_each do |opt|
      expect { sharing.update_attributes(share_by_default: opt) }
                          .to change { sharing.share_by_default }.to(opt)
    end
  end

  it 'should allow you to update :reminders_frequency' do
    Sharing.reminders_frequencies.keys.reverse_each do |freq|
